...
namespace Admin
{
    public class Startup
    {
        ...
        {
            //for mongo db to treat guid and datetime in the format we require
            BsonSerializer.RegisterSerializer(new GuidSerializer(BsonType.String));
            BsonSerializer.RegisterSerializer(new DateTimeOffsetSerializer(BsonType.String));

            services.AddSingleton<IMongoClient>(options => {
                //map the section from appconfig to the class created and create required to fill connection string property(host & port)
